...
public class EmployeeDao {
	...
	public void insertEmp(Employee emp, Salary sal) {
		...
		try {
			ps=con.prepareStatement("insert into mphemp(eid,ename,salary) values(?,?,?)");
	...
	public void type_scroll_sensitive_rs_update(Employee emp,Salary sal)
	{
		con = MyDBConnection.getDBConnection();
		try {
			ps = con.prepareStatement("select eid,ename,salary from mphemp",ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_UPDATABLE);
			rs = ps.executeQuery();
			...
			ps=con.prepareStatement("update mphemp set ename='Test',salary=2000 where eid=1 ");
			rs =ps.executeQuery();
			//rs.updateInt("eid",3);
			//rs.updateString("ename", "SS");
			//rs.updateRow();
			
			System.out.println("Updated ...");
			System.out.println("After Update ==>");
			
			ps = con.prepareStatement("select eid,ename,salary from mphemp",ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
			rs = ps.executeQuery();
	...
	public void batchUpdate() {
		...
		try {
			st= con.createStatement();
			st.addBatch("update mphemp set ename='B1' where eid=1");
			st.addBatch("update mphemp set ename='B2' where eid=2");
			st.addBatch("update mphemp set eid=11 where ename='SS'");
